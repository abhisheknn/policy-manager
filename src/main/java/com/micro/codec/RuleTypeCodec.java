...
public class RuleTypeCodec extends TypeCodec<Rule>{
	...
	private UDTValue toUDTValue(Rule rule) {
		...
		if(null!=rule) {
			...
			.setBool("isprivate", rule.getIsPrivate())
			.setString("ruleexpression", rule.getRuleExpression())
			.setString("ruleinterpreter", rule.getRuleInterpreter())
			.setString("rulename", rule.getRuleName())
			.setString("ruletype", rule.getRuleType())
			.setString("tenant", rule.getTenant());
		}
		return udtvalue;
	}
